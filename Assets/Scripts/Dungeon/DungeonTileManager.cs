...
{
    private Dictionary<Vector3Int, Coroutine> flipCoroutines = new Dictionary<Vector3Int, Coroutine>();

    [Header("Tilemap")]
    public Tilemap dungeonTilemap;
    
    [Header("Tiles")]
    public AnimatedTile fogTile; // 미탐색 타일
    public AnimatedTile defaultFloorTile; // Empty, Start 타일용
    public AnimatedTile playerTile;
    public AnimatedTile pathTile;

    [Header("Event Tiles")]
    public AnimatedTile normalMonsterTile;
    ...
    public AnimatedTile GetRevealedTile(HexTileData tileData)
    {
    ...
    public void FlipToTile(Vector3Int pos, AnimatedTile targetAnimTile, System.Action onComplete = null)
    {
        ...
        Coroutine routine = StartCoroutine(PlayFlipAnimation(pos, currentAnimTile, targetAnimTile, () => {
            flipCoroutines.Remove(pos);
            onComplete?.Invoke();
        }));
        
        flipCoroutines[pos] = routine;
    }

    private System.Collections.IEnumerator PlayFlipAnimation(Vector3Int pos, AnimatedTile currentAnimTile, AnimatedTile targetAnimTile, System.Action onComplete = null)
    {
        float aniLimitTime = 0.05f;

        // 1. 접히는 과정 (Frame 1 ~ 3) - 현재 타일 사용
        // 만약 현재 타일이 AnimatedTile이 아니거나 프레임이 부족하면 예외 처리
        for (int i = 1; i <= 3; i++)
        {
            dungeonTilemap.SetAnimationFrame(pos, i);
            yield return new WaitForSeconds(aniLimitTime);
        }

        // 2. 펴지는 과정 (Frame 4 ~ 5) - 목표 타일로 교체!
        dungeonTilemap.SetTile(pos, targetAnimTile);
        for (int i = 4; i < 6; i++)
        {
            dungeonTilemap.SetAnimationFrame(pos, i);
            yield return new WaitForSeconds(aniLimitTime);
